...
class InfoCard extends StatefulWidget {
  ...
  final List<LodgeData> newlodgeData;

  const InfoCard(
      {Key? key, required this.activeproducts, required this.totalproducts, required this.newlodgeData})
      : super(key: key);
...
class _InfoCardState extends State<InfoCard> {
        ...
        child: Stack(
          children: [
            ...
            Positioned(
              ...
              bottom: 30,
              child: Text(
                widget.activeproducts.toString(),
                style: TextStyle(
                  fontSize: 30,
                  fontWeight: FontWeight.w600,
                  color: kPrimary1,
  ...
  getDateRange() {
    try {
      ...
      setState(() {
        day1 = widget.newlodgeData.where((element) => element.dateCreated! == distinctDate[0]).toList();
        day2 = widget.newlodgeData.where((element) => element.dateCreated! == distinctDate[1]).toList();
        day3 = widget.newlodgeData.where((element) => element.dateCreated! == distinctDate[2]).toList();
        day4 = widget.newlodgeData.where((element) => element.dateCreated! == distinctDate[3]).toList();
        day5 = widget.newlodgeData.where((element) => element.dateCreated! == distinctDate[4]).toList();
        day6 = widget.newlodgeData.where((element) => element.dateCreated! == distinctDate[5]).toList();
        day7 = widget.newlodgeData.where((element) => element.dateCreated! == distinctDate[6]).toList();
      });
      setState(() {
        listData!.add(Data(
            id: 0,
    ...
    // setState(() {
    //   widget.newlodgeData.sort(
    //     (a, b) => a.date!.toDate().compareTo(b.date!.toDate()),
    //   );
    // });
    // DateTime min = widget.newlodgeData.first.date!.toDate();
    // DateTime max = widget.newlodgeData.last.date!.toDate();

    // for (int i = 0; i < min.add(Duration(days: i)).difference(max).inDays; i++) {
    //   setState(() {});
    // }
  }
}
