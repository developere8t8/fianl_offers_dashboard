...
class EditProduct extends StatefulWidget {
  bool? edit;
  LodgeData lodgedata;
  EditProduct({Key? key, this.edit, required this.lodgedata}) : super(key: key);
...
class _EditProductState extends State<EditProduct> {
  ...
  void initState() {
    ...
    selectUni1 = unit1[0];
    selectUni2 = unit2[0];
    name.text = widget.lodgedata.name!;
    for (var i in widget.lodgedata.photos!) {
      photoUrls.add(i);
    }
    video.text = widget.lodgedata.videoUrl!;
    description.text = widget.lodgedata.description!;
    if (widget.lodgedata.amentities['Cell Reception']) {
      amentitesData.add(
          AmentitesData(item: 'Cell Reception', present: widget.lodgedata.amentities['Cell Reception']));
    }
    if (widget.lodgedata.amentities['Pet Friendly']) {
      amentitesData.add(
          AmentitesData(item: 'Pet Friendly', present: widget.lodgedata.amentities['Pet Friendly']));
    }
    if (widget.lodgedata.amentities['Wifi']) {
      amentitesData.add(AmentitesData(item: 'Wifi', present: widget.lodgedata.amentities['Wifi']));
    }

    from = widget.lodgedata.from!.toDate();
    to = widget.lodgedata.to!.toDate();
    permanant = widget.lodgedata.permanent!;
    selectedValue = widget.lodgedata.region!;
    address.text = widget.lodgedata.location!;
    selectUni1 = widget.lodgedata.unit!;
    selectUni2 = widget.lodgedata.duration!;
    category = widget.lodgedata.category!;
    priceC.text = widget.lodgedata.price!.toString();
    terms.text = widget.lodgedata.terms!;
    super.initState();
  }
                                  ...
                                  children: [
                                    SizedBox(
                                      ...
                                      child: ListView.builder(
                                          itemCount: photoUrls.isEmpty ? 0 : photoUrls.length,
                                          shrinkWrap: true,
                                          primary: true,
                        ...
                        child: Center(
                          ...
                          child: FixedPrimary(
                              ...
                              ontap: () {
                                ...
                                } else if (imgs.length + photoUrls.length < 3) {
...
//picking image
  ...
  Future savingLodge() async {
    ...
    try {
      final newLodge = FirebaseFirestore.instance.collection('lodges').doc(widget.lodgedata.id);
      //uploading Images
      List imgUrls = [];
      for (var i in imgs) {
      ...
      LodgeData data = LodgeData(
          ...
          videoUrl: video.text,
          bookings: widget.lodgedata.bookings,
          ranking: widget.lodgedata.ranking,
          reviews: widget.lodgedata.reviews,
          latitude: lat,
          longitude: long,
          date: Timestamp.fromDate(DateTime.now()),
          status: widget.lodgedata.status,
          category: category,
          dateCreated: DateFormat('yyyy-MM-dd').format(DateTime.now()),
          adminStatus: widget.lodgedata.adminStatus,
          archive: false);

      await newLodge.update(data.toMap()).whenComplete(() {
        setState(() {
          isLoading = false;
        });
          ...
          MaterialPageRoute(
              builder: (context) => ErrorDialog(
                  title: 'Succes',
                  message: 'Your product is saved Ssuccessfully',
                  type: 'E',
                  function: () {
